...
class RequestView(AdminFormView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['amount'] = forms.IntegerField(label=_('How many of "{item}{notes}" would you like?').format(
            item=self.item,
            notes=f" ({self.item.notes})" if self.item.notes else ""),
            initial=1, min_value=1, max_value=self.item.available)
        return form

    def form_valid(self, form):
        today = timezone.now()
        amount = form.cleaned_data['amount']
        request, new = Request.objects.get_or_create(contact=self.request.user,
                                                     goal=f"Requests of {today:%Y-%m-%d}")
        item = RequestItem.objects.create(request=request,
                                          type=self.item.type,
                                          brand=self.item.brand,
                                          model=self.item.model,
                                          notes=self.item.notes,
                                          amount=amount)
        Claim.objects.create(offered_item=self.item,
                             requested_item=item,
                             amount=amount)

        messages.info(self.request, _("Request for {amount}x {item} created").format(
            amount=amount,
            item=self.item,
        ))
