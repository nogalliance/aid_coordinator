...
class ContactAdmin(UserAdmin):
    ...
    def get_urls(self):
        return [path("email/", self.admin_site.admin_view(EmailView.as_view()))] + super().get_urls()
    ...
    def send_welcome_email(self, request: HttpRequest, queryset: Contact.objects):
        ...
        for contact in queryset:
            if contact.is_superuser:
                self.message_user(
                    request,
                    f"Not sending a message to superuser {contact}",
            ...
            if not contact.group_names:
                self.message_user(
                    request,
                    f"{contact} is not in a group, not sending welcome message",
    ...
    def send_custom_email(self, request: HttpRequest, queryset: Contact.objects):
        ...
        return HttpResponseRedirect("email/?contacts=%s" % (",".join(str(pk) for pk in selected),))
    ...
    def admin_groups(self, contact: Contact):
        ...
        return format_html_join(", ", "{}", [(str(group),) for group in contact.groups.all()])
    ...
    def admin_email(self, contact: Contact):
        return format_html(
            '<a href="mailto:{name} %3c{email}%3e">{email}</a>',
    ...
    def get_queryset(self, request):
        ...
        if request.user.organisation_id:
            return queryset.filter(Q(organisation_id=request.user.organisation_id) | Q(pk=request.user.pk))
